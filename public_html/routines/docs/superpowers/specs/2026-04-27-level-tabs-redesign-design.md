...
**한 글당 한 번만 streak +1** (어느 레벨로 6/6 했는지 무관).

- 구현: `POST /api/progress` 에서 6/6 done 검증 통과 후, `analytics_events` 에서 `type='complete' AND contentId=X AND userId=Y` 가 이미 있는지 확인. 있으면 streak skip, `complete` 이벤트는 metadata 만 업데이트 (또는 추가 기록 안 함). 없으면 streak +1 + `complete` 이벤트 기록.
- 이유: streak 는 "오늘 학습했는가" 에 가까운 지표. 같은 글을 여러 레벨로 푼 건 학습량은 늘었지만 "오늘 한 글 = 1 카운트" 가 더 직관적.
- 메모: streak 기능 자체는 현재 사용자에게 노출되지 않지만, 데이터 정확성을 위해 정책 확정해 둠.
...
| 5 | Streak 중복 증가 방지 | `analytics_events` 에서 `type='complete' AND contentId=X AND userId=Y` 이미 있으면 streak skip. Streak 정책 섹션 참조 |
